...
#define CONFIG_CMD_MAX                                                         \
	(PORTS_MAX * 6 +                                                       \
	 18) // 6 is max char num. strlen("xxxxx,"). 18 used for CMD_PORTS
...
struct sla_interface_stats //16 bytes + circular buffer
{
	...
	struct sla_stats_entry entries
		[ENTRY_TOTAL]; //pointer of first entry, alse circular buffer's head pointer
};
