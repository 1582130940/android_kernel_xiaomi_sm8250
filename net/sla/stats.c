...
static int sla_interface_start(const char *interface_name)
{
	...
	if (NULL == interface_name) {
		printk(KERN_CRIT
		       "### %s Cannot create iface struct when iface name is NULL \n",
		       __func__);
		return -1;
	}
	STATS_LOCK_IRQ(&stats_spin_lock, flags);
	if (sla_find_interface(interface_name) != NULL) {
		STATS_UNLOCK_IRQ(&stats_spin_lock, flags);
		printk(KERN_CRIT
		       "### %s Interface %s already being tracked in SLA ",
		       __func__, interface_name);
		return -1;
	}
	STATS_UNLOCK_IRQ(&stats_spin_lock, flags);

	new_interface = (struct sla_interface *)kmalloc(
		sizeof(struct sla_interface), GFP_ATOMIC);

	if (NULL == new_interface) {
		printk(KERN_CRIT "### %s kmalloc Failed for %s", __func__,
		       interface_name);
		return -1;
	}
	...
	if (strlcpy(new_interface->interface_name, interface_name,
		...
		printk(KERN_CRIT "### %s invalid interface name %s", __func__,
		       interface_name);
		kfree(new_interface);
		return -1;
	}
	...
	new_interface->mmap_struct = (struct sla_interface_stats *)kmalloc(
		STATS_BUF_SIZE, GFP_ATOMIC);
	...
	if (!new_interface->mmap_file) {
		printk(KERN_ERR
		       "#### %s Fail to create file: mmap_file for interface= %s \n",
		       __func__, interface_name);
	...
	if (!new_interface->total_stats_file) {
		printk(KERN_ERR
		       "#### %s Fail to create total_stats_fils for interface = %s \n",
		       __func__, interface_name);
				 ...
				 size_t count, loff_t *ppos)
{
	...
	if (0 == strncmp(cmd, CMD_MAX_SIZE, len)) {
		...
		if (num < MAX_SIZE_DEFAULT) {
			printk(KERN_ERR
			       "#### %s max_size=%lu is invaild, need greater than %d\n",
	...
	if (0 == strncmp(cmd, CMD_LIMIT_TIME, len)) {
		...
		if (num < LIMIT_TIME_DEFAULT) {
			printk(KERN_ERR
			       "#### %s limit_time=%lu is invaild, need longer than %d\n",
