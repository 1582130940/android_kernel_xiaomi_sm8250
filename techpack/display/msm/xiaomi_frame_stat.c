...
void calc_fps(u64 duration, int input_event)
{
	...
	idle_fps = g_panel->mi_cfg.idle_fps ? (u64)g_panel->mi_cfg.idle_fps :
					      IDLE_FPS;
	...
	if (fm_stat.last_frame_commit_time_us > 0) {
		...
		if (diff > LONG_FRAME_INTERVAL) {
			fm_stat.skip_count++;
			pr_debug(
				"%s: Long frame interval, frame interval[%lld ms], count[%lld]\n",
	...
	if (diff_us >= FPS_PERIOD_1_SEC) {
		...
		pr_debug(
			"%s: FPS:%d.%d max_frame_duration = %lld(us) max_input_fence_duration = %lld(us)\n",
		...
		if (fm_stat.max_frame_duration < fm_stat.delta_commit_duration)
			fm_stat.max_frame_duration =
				fm_stat.delta_commit_duration;

		fm_stat.delta_input_duration = fm_stat.input_fence_duration;
		if (fm_stat.max_input_fence_duration <
		    fm_stat.delta_input_duration)
			fm_stat.max_input_fence_duration =
				fm_stat.delta_input_duration;
...
void frame_stat_collector(u64 duration, enum stat_item item)
{
	...
	case COMMIT_END_TS:
		...
		if (fm_stat.input_fence_duration > duration / 10)
			pr_debug(
				"%s: long wait for input fence might cause frame miss!\n",
				__func__);
		calc_fps(duration, 0);
		break;
	default:
