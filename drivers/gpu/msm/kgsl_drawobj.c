...
struct kgsl_drawobj_cmd *kgsl_drawobj_cmd_create(struct kgsl_device *device,
		...
		unsigned int type)
{
	struct kgsl_drawobj_cmd *cmdobj = kmem_cache_zalloc(cmd_cache, GFP_KERNEL);
	int ret;
...
int kgsl_drawobjs_cache_init(void)
{
	...
	if (!memobjs_cache || !sparseobjs_cache || !timeline_cache || !sparse_cache || !sync_cache || !cmd_cache)
		return -ENOMEM;
