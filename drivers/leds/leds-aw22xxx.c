...
static int aw22xxx_brightness_store(struct led_classdev *cdv,
				    enum led_brightness value)
{
	...
	if (!strcmp(cdv->name, "red")) {
		...
		aw22xxx_led_rgb_code[AW22XXX_BRIGHTNESS_INDEX] =
			(unsigned char)value;
		aw22xxx->red_cdev.brightness = (unsigned char)value;
		for (i = 0; i < sizeof(aw22xxx_led_rgb_code); i += 2) {
			aw22xxx_i2c_write(aw22xxx, aw22xxx_led_rgb_code[i],
					  aw22xxx_led_rgb_code[i + 1]);
		}
	}
...
static int aw22xxx_blink_set(struct led_classdev *cdv, unsigned long *delay_on,
			     unsigned long *delay_off)
{
	...
	aw22xxx_led_blink_code[AW22XXX_DELAY_ON_LOW] =
		(unsigned char)*delay_on & 0xff;
	aw22xxx_led_blink_code[AW22XXX_DELAY_ON_HIGH] =
		(unsigned char)(*delay_on >> 8) & 0xff;
	aw22xxx_led_blink_code[AW22XXX_DELAY_OFF_LOW] =
		(unsigned char)*delay_off & 0xff;
	aw22xxx_led_blink_code[AW22XXX_DELAY_OFF_HIGH] =
		(unsigned char)(*delay_off >> 8) & 0xff;

	if (!strcmp(cdv->name, "red")) {
		pr_info("aw22xxx set blink of red \n");
		aw22xxx = container_of(cdv, struct aw22xxx, red_cdev);
		for (i = 0; i < sizeof(aw22xxx_led_blink_code); i += 2) {
			aw22xxx_i2c_write(aw22xxx, aw22xxx_led_blink_code[i],
					  aw22xxx_led_blink_code[i + 1]);
		}
	} else if (!strcmp(cdv->name, "green")) {
