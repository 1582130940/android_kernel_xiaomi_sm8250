...
static int detect_gpio_status_timeout(struct gamekey_core *core, int gpio_nr,
				      int ms);

static ssize_t gamekey_read(struct file *filp, char __user *buf, size_t count,
			    loff_t *ppos)
{
	int ret = 0;
	char *sts = g_core->sts;
	...
	pr_debug("%s, ret=%d, Left H:%d, K:%d   Righ H:%d, K:%d", __func__, ret,
		 sts[HALL_LEFT], sts[GAMEKEY_LEFT], sts[HALL_RIGHT],
		 sts[GAMEKEY_RIGHT]);
...
static int parse_dt(struct gamekey_core *core)
{
	...
	core->hw.gpio_info[HALL_LEFT].gpio =
		of_get_named_gpio_flags(np, "hall_left", 0, NULL);
	...
	core->hw.gpio_info[HALL_RIGHT].gpio =
		of_get_named_gpio_flags(np, "hall_right", 0, NULL);
	...
	core->hw.gpio_info[GAMEKEY_LEFT].gpio =
		of_get_named_gpio_flags(np, "key_left", 0, NULL);
...
static irqreturn_t hall_left_handler(int irq, void *data)
{
	...
	if (core->sts[HALL_LEFT] >= 0) {
		input_report_switch(input_dev, SW_HALL_LEFT,
				    core->sts[HALL_LEFT]);
...
static irqreturn_t hall_right_handler(int irq, void *data)
{
	...
	if (core->sts[HALL_RIGHT] >= 0) {
		input_report_switch(input_dev, SW_HALL_RIGHT,
				    core->sts[HALL_RIGHT]);
		input_sync(core->input_dev);
		core->event_update = true;
		wake_up_interruptible(&event_wait);
...
static irqreturn_t key_right_handler(int irq, void *data)
{
	...
	if (core->sts[GAMEKEY_RIGHT] >= 0) {
		input_event(input_dev, EV_KEY, KEY_F2,
			    core->sts[GAMEKEY_RIGHT]);
		input_sync(core->input_dev);
		core->event_update = true;
		wake_up_interruptible(&event_wait);
	}
...
static int gpio_init(struct gamekey_core *core)
{
	...
	*/
	...
	ret = pinctrl_select_state(core->gamekey_pinctrl,
				   core->pinctrl_state_active);
	if (ret) {
...
static int irq_init(struct gamekey_core *core)
{
	...
	if ((hw->hall_l_irq > 0) && (hw->hall_r_irq > 0) &&
		...
		ret = request_threaded_irq(
			hw->hall_l_irq, NULL, hall_left_handler,
		...
		ret = request_threaded_irq(
			hw->hall_r_irq, NULL, hall_right_handler,
			IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING |
				IRQF_ONESHOT,
			"hall_r_irq", core);
		if (ret) {
		...
		ret = request_threaded_irq(
			hw->key_l_irq, NULL, key_left_handler,
			IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING |
				IRQF_ONESHOT,
			"key_l_irq", core);
		if (ret) {
		...
		ret = request_threaded_irq(
			hw->key_r_irq, NULL, key_right_handler,
			IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING |
				IRQF_ONESHOT,
			"key_r_irq", core);
		if (ret) {
		...
		pr_err("irq number illegal, hall left: %d,\
		       ...
		       hw->hall_l_irq, hw->hall_r_irq, hw->key_l_irq,
		       hw->key_r_irq);
		goto irqno_err;
	}
...
static int get_status_after_boot(struct gamekey_core *core)
{
	...
	pr_debug("%s, Left H:%d, K:%d   Righ H:%d, K:%d", __func__,
		 core->sts[HALL_LEFT], core->sts[GAMEKEY_LEFT],
