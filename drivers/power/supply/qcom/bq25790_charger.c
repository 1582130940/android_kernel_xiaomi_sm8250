...
static int power_supply_set_present(struct power_supply *usb_psy, bool enable)
{
	...
	ret = power_supply_set_property(usb_psy, POWER_SUPPLY_PROP_PRESENT,
					&prop);
	if (ret < 0) {
...
static int bq25790_charging_disable(struct bq25790 *bq, int reason, int disable)
{
	int ret = 0;
	int disabled;
...
static int bq25790_get_prop_batt_present(struct bq25790 *bq)
{
	...
	ret = bq25790_get_property_from_bms(bq, POWER_SUPPLY_PROP_PRESENT,
					    &batt_prop);
	if (!ret)
...
static void bq25790_external_power_changed(struct power_supply *psy)
{
	...
	ret = power_supply_get_property(bq->usb_psy, POWER_SUPPLY_PROP_ONLINE,
					&prop);
	if (ret < 0)
	...
	if (bq->usb_present &&
	    bq->charge_state != BQ25790_CHRG_STAT_NOT_CHARGING) {
		if (prop.intval == 0) {
...
static void bq25790_check_batt_pres(struct bq25790 *bq)
{
	...
	if (!ret) {
		chg_disabled_pres =
			!!(bq->charging_disabled_status & BATT_PRES);
		if (chg_disabled_pres ^ !bq->batt_present) {
			ret = bq25790_charging_disable(bq, BATT_PRES,
						       !bq->batt_present);
			if (ret) {
...
static void bq25790_check_batt_full(struct bq25790 *bq)
{
	...
	if (!ret) {
		...
		if (chg_disabled_fc ^ bq->batt_full) {
			ret = bq25790_charging_disable(bq, BATT_FC,
						       bq->batt_full);
			if (ret) {
				bq_err("failed to %s charging, ret = %d\n",
