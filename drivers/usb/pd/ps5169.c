...
static int ps5169_get_chipid_revision(struct ps5169_info *info)
{
	...
	ret |= ps5169_read_reg(info, REG_REVISION_L, &revision_l);
	pr_info("%s: Revision: 0x%02x, 0x%02x", __func__, revision_h,
		revision_l);
...
static void ps5169_enable_work(struct work_struct *work)
{
	...
	if (info->ps_enable) {
		ret = pinctrl_select_state(info->ps5169_pinctrl,
...
static enum power_supply_property ps5169_props[] = {
	POWER_SUPPLY_PROP_PS_EN,	 POWER_SUPPLY_PROP_PS_CHIPID,
	POWER_SUPPLY_PROP_PS_CFGMOD,	 POWER_SUPPLY_PROP_PS_CFG_FLIP,
	POWER_SUPPLY_PROP_PS_CFG_USB,	 POWER_SUPPLY_PROP_PS_CFG_DP,
	POWER_SUPPLY_PROP_PS_CFG_USB_DP, POWER_SUPPLY_PROP_PS_RMOV_USB,
	POWER_SUPPLY_PROP_PS_RMOV_DP,	 POWER_SUPPLY_PROP_PS_RMOV_USB_DP,
	POWER_SUPPLY_PROP_EQ0_TX,	 POWER_SUPPLY_PROP_EQ1_TX,
	POWER_SUPPLY_PROP_EQ2_TX,	 POWER_SUPPLY_PROP_TX_GAIN,
};
...
static int ps5169_init_psy(struct ps5169_info *info)
{
	...
	info->ps_psy = devm_power_supply_register(info->dev, &info->ps_psy_desc,
						  &ps_cfg);
