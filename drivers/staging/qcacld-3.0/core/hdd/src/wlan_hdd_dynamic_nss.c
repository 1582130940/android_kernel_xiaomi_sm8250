...
int wlan_hdd_set_nss_and_antenna_mode(struct hdd_adapter *adapter, int nss,
				      int mode)
{
	QDF_STATUS status;
	...
	if (((nss == NSS_1x1_MODE) && (mode == HDD_ANTENNA_MODE_2X2)) ||
		...
		hdd_err("nss and antenna mode is not match: nss = %d, mode = %d",
			nss, mode);
		return -EINVAL;
	}
	...
	if (QDF_IS_STATUS_ERROR(status)) {
		hdd_err("cfg set antenna mode failed, value %d status %d", mode,
			status);
		return qdf_status_to_os_return(status);
	}
